...
class User extends BaseController{
    ...
    public function examine_list(){
        ...
        if(!$user_list->isEmpty()){
            return ajax_success($user_list,'获取需审核用户成功!');
        }else{
    ...
    public function examine_details(){
        ...
        $user_det['rec_id'] = \app\Models\User::where('id',$user_det['rec_id'])->value('name');
        $user_det['gen_id'] = \app\Models\User::where('id',$user_det['gen_id'])->value('name');
        if(empty($user_det)){
    ...
    */
    public function del_user(){
        ...
        $user = \app\Models\User::where('id',$user_id)->delete();
        if($user){
            return ajax_success(200,'驳回成功');
        }else{
            return ajax_error(400,'博会失败');
        }
    }
