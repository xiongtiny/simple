...
// 用户界面
    ...
    if(!$user_id){
        $this->error('','/api/v2/index/login1');
    }else{
        $user = \app\model\User::where('id',$user_id)->find();
    ...
    public function feedback(){
        ...
        if(request()->isPost()){
            ...
            if($feedback->id){
                return success(200,'反馈意见已提交',$feedback->id);
            }else{
                return error(400,'反馈意见提交失败');
            }
        }
     ...
     * @return \think\response\Json
     */
    public function user_release1()
    {
        return $this->fetch();
    }
    public function user_release(){
        $user_id = Session::get('user_id');
        $user_farm = \app\model\Farm::where('user_id',$user_id)->order('type')->select();
        // dump($user_farm);exit;
    ...
    public function user_lease(){
        $user_id = Session::get('user_id');
        $user_farm = Order::where('user_id',$user_id)->whereIn('type',[1,3])->select();
        if(!$user_farm->isEmpty()){
            foreach ($user_farm as $v){
                $v['start_time'] = explode(' ',$v['create_time'])[0];
    ...
    public function trade(){
        ...
        // $num=5;//每页数量
        $user_id = Session::get('user_id');
        $user_record = \app\model\Order::withTrashed()->where('user_id',$user_id)->whereIn('type',[1,2,3])->select(); 
        if(!$user_record->isEmpty()){
            foreach ($user_record as $v){               
                $v['farm_name'] = \app\model\Farm::withTrashed()->where('id',$v['farm_id'])->value('name');
                if($v['type']==1||$v['type']==3){ $v['type'] = '租赁农场';}
                if($v['type']==2){ $v['type'] = '租出农场';}
            }           
        } 
    ...
    //         $user_id = Session::get('user_id');
    //         $user_record = \app\model\Order::withTrashed()->where('user_id',$user_id)->limit($num)->page($page)->select();
    //         $pages = $all->count();
    //         $pages = ceil($pages/$num);
    //         if(!$user_record->isEmpty()){
    //             foreach ($user_record as $v){               
    //                 $v['farm_name'] = \app\model\Farm::withTrashed()->where('id',$v['farm_id'])->value('name');
    //                 if($v['type']==1){
    ...
    //         json(['data'=>$user_record,
    //             'pages'=>$all->count()])->send();
    //         exit;
