...
class Farm extends Controller{
    ...
    public function farm_list(){
        $farm_data = \app\model\Farm::where('type',0)->paginate(10);
    ...
    public function farm_detail(){
        ...
        // //判断农场是否被租赁
         $farm_state = Order::where('user_id',$id)
        ...
        if(!request()->get('openid')){
            ...
            $this->redirect('http://www.fineclab.com/api/wechat/index');
        }
        //获取农场租赁价格
    ...
    public function edit_farm(){
        ...
        // $farm_data['service'] = Service::whereIn('id',$farm_data['service'])->column('name');
        if(request()->isPost()){
            $data = request()->post();
            if(request()->file('img')){
