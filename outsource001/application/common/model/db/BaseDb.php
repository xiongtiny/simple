...
Class BaseDb extends Model{
	...
	public function getAll($where = array(), $order_by = array(), $field = ''){
        ...
        $ret    = $field ? $this->field($field)->select() : $this->select();
    ...
    public function addInfo($data = array(), $is_batch = false){
        ...
        $ret    = $is_batch ? $this->saveAll($data) : $this->insertGetId($data);
    ...
    public function getInfo($where = array(), $field=''){
        ...
        if(!empty($field))
        {
            if(strpos($field, ',') !== false){
                $ret1   = $this->field($field)->select();
                $ret    = isset($ret1[0])?$ret1[0]:'';
            }else{
                $ret    = $this->value($field);
            }
