...
class  User extends  Base{
    ...
    public function  userList(){
        ...
        $users=UserModel::where( $arrWhere)->with('getPro')->with('getCity')->with('getArea')->with('getRecName')->with('getGenName')->paginate('10');
        // dump($users);exit;
        $count = UserModel::where( $arrWhere)->with('getPro')->with('getCity')->with('getArea')->with('getRecName')->with('getGenName')->count();
      $page=$users->render();
...
//        dump($re);exit;
         if($re){
             return  $this->success('添加成功','/admin/v1/user/userList');
         }else{
...
//       dump($id);exit;
           //获取商品名
       $model =new Special;
       $specials=$model->where('user_id',$id)->with('getName')->with  ('getGoods')->paginate(10,false,['query'=>request()->param()]);;
//     dump($specials);exit;
...
//             dump($re);exit;
             if($re){
                  return $this->success('修改成功','/admin/v1/user/userList');
             }else{
...
//          //获取当前用户的id
         $user_id = UserModel::where('phone',$phone)->value('id');
          /* @实例化 */
          $excel = new \PHPExcel();
          ...
          //获取当前总代下面的所有人
          $user = UserModel::where('gen_id',$user_id )->order('rec_id','desc')->select();
          foreach ($user as $v){
              $v['name'] = $v['name'];
              $v['rec_name'] = UserModel::where('id',$v['rec_id'])->value('name');
          }
           $arr=[];
          foreach ($user as $k=>$v){
              $arr[$k]['a']=$v['name'];
...
//写入多行数据
          ...
          //获取当前总代下面的所有人
          $users = UserModel::where('gen_id',$user_id)->where('rec_id','<>',$user_id)->order('rec_id','desc')->select();
          foreach ($users as $v){
              $v['name'] = $v['name'];
              $v['rec_rebate'] = \app\Models\Order::where('user_id',$v['id'])->where('status',3)->sum('rec_rebate');
              $v['rec_name'] = UserModel::where('id',$v['rec_id'])->value('name');
          }
          $arr1=[];
...
//写入多行数据
          ...
          //获取当前总代下的所有用户
          $user_data = UserModel::where('gen_id',$user_id)->order('rec_id','desc')->select();
          foreach ($user_data as $k => $v){
              $user_data[$k]['ids'] = UserModel::where('rec_id',$v['id'])->column('id');
              $v['price'] = \app\Models\Order::whereIn('user_id',$user_data[$k]['ids'])->where('status',3)->sum('rec_rebate');
          }
          $arr2=[];
          foreach ($user_data as $k=>$v){
              $arr2[$k]['a']=$v['name'];
          ...
          header("Content-Disposition:attachment;filename=".$time."月返利排行表.xls");
...
//     }
    ...
    */
    public function upgrade(){
        ...
        $day=date('Y-m-d',time());
        if($day==$lastDay){            
            $user_under = UserModel::where('rec_id',$id)->column('id');
            $under_id = $this->recursion($id);   
            // 启动事务         
            Db::startTrans();
            try {
                $model= new UserModel;
                ...
                $save_user = UserModel::whereIn('id',$under_id)->update(['gen_id'=>$id]);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
              dump($e->getMessage());
            ...
            if($model){
                return $this->success('操作成功','/admin/v1/user/userList','',1);
            }else{
                return $this->error('操作失败','/admin/v1/user/userList','',1);
            }
        }else{
            return $this->error('请在本月最后一天进行升级','/admin/v1/user/userList','',1);
        }
    }
    ...
    public function cancel(){
        ...
        if($day==$lastDay){
                 $model=new UserModel;
                 $gen_id=$model->where('id',$id)->value('gen_id');
                 $re=$model->save(['rec_id'=>$gen_id],['id'=>$id]);
                 if($re){
                    return $this->success('操作成功','/admin/v1/user/userList','',1);
                 }else{
                     return $this->error('操作失败','/admin/v1/user/userList','',1);
            ...
            return  $this->error('请在本月最后一天进行操作','/admin/v1/user/userList','',1);
        }
    }
     ...
     * 递归查询所有下级
     */
    public function recursion($rec_id){
      static $b=array();
      $a = UserModel::where('rec_id', $rec_id)->where('grade',1)->column('id');
      if(!empty($a)){
